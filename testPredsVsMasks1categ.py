...
def get_metrics_init(y_true, y_pred, loss=float('nan')):
...
#folders to check
path_test_im = './test_images' #3053  test_images
path_test_mask = './test_masks_liver'#test_masks_liver
path_pred_mask = './pred_masks_liver_segmentation_resnet50ep40_CrossEntropy_diceMetrics' # pred_masks_liver_trial_100ep
plot_title_preffix = 'Liver segmentation'
#f = open('resultsTumSegmFromAllCts.csv',mode = 'w')
f = open(f"results-{path_pred_mask.replace('./', '')}.csv",mode = 'w') 
f.write('liverFile, maskImg1s, predMaskImg1s, Accuracy, TP, TN, FP, FN, Precision, Recall, F1_score, IOU, DSC, FNR, FPR, Sens, Spec, MaskContours, PredMaskContours \n')

conf_matrix_pixel =  np.zeros((2,2),dtype = int) #to create confusion matrix to see performance for liver n tumor detection per sample based on pixel instance
conf_matrix_contours =  np.zeros((2,2),dtype = int) #to create confusion matrix to see performance for liver n  tumor detection per sample based on contours

imgDim = (512, 512) #dim mask Images should be compared, 512 original sizes, convert pred 128-->512

#%%loop start
counter=0
file_iterator = os.scandir(f'{path_test_im}')
for i in file_iterator:
    #liverImg = cv2.imread("./test_images/"+i.name,-1)
    liverFile = i.name

    maskFile = predMaskFile = liverFile.split(".")[0] + "_mask.png"
    
    maskImg = cv2.imread(f'{path_test_mask}/{maskFile}',0)  # volume-87_slice_497_mask.png (imread(fname, mode), mode: 0 for grayscale, 1 color, -1 unchanged, nothing: brings all three in array)
    predMaskImg = cv2.imread(f'{path_pred_mask}/{predMaskFile}', 0) #volume-87_slice_497_pred_mask.png
    ...
    if maskImg is None: # if no mask image found means a full of zeroes matrix, no livenr, no tumor
        maskImg = np.zeros(imgDim)
    
    #%% resize image prediction mask (128x128) to fit to originals (they are 512x512) 
    #predMaskImg = cv2.resize(predMaskImg, imgDim, interpolation = cv2.INTER_AREA) #it is already
    ...
    metrics = get_metrics(npmaskImg, nppredMaskImg) # get metrics from comparing pred and ground truth
   
    #%%check contours for exiatance of liver or tumor
    #check tumor-liver by contours in test mask
    maskOnlyBackground = predMaskOnlyBackground = maskContours = predMaskContours = 0
    if np.count_nonzero(maskImg)==0: #["background","liver","tumor"] if all zero we have only background
        ...
        # for liver
        Img_mask = np.where(maskImg == 1, 255, maskImg) # if 1 liver, make 255, else get value of img, img_liver is the liver mask
        ret = check(Img_mask) # check image for liver contours, if find, next, write 1 in second bit for liver found
        if ret: 
            maskContours =1
        else:
            maskContours =0
    ...
    if np.count_nonzero(predMaskImg)==0: #["background","liver","tumor"] if all zero we have only background
        ...
        # for liver
        Img_pred_mask= np.where(predMaskImg == 1, 255, predMaskImg) # if 1 liver, make 255, else get value of img, img_liver is the liver mask
        ret = check(Img_pred_mask) # check image for liver contours, if find, next, write 1 in second bit for liver found
        if ret: 
            predMaskContours =1
        else:
            predMaskContours =0
    ...
    f.write(liverFile + ', ' + str(npmaskImg1s) + ', ' + str(nppredMaskImg1s) + ', '  + str(metrics["accuracy"]) +', ' + str(metrics["TP"]) +', ' + str(metrics["TN"])+', ' + str(metrics["FP"])+', ' + str(metrics["FN"])+ ', ' + str(metrics["precision"])+', ' + str(metrics["recall"]) +', ' + str(metrics["f1_score"])+', ' + str(metrics["iou"])+', ' + str(metrics["DSC"])+', ' +  str(metrics["FNR"])+', ' +  str(metrics["FPR"])+', ' + str(metrics["Sens"])+', ' + str(metrics["Spec"]))
    f.write(', ' + str(maskContours) + ', ' + str(predMaskContours) + '\n')

    #conf matrix finding by  pixel existance
    if nppredMaskImg1s > 0  and npmaskImg1s > 0:
        conf_matrix_pixel[0,0] += 1
    if nppredMaskImg1s == 0 and npmaskImg1s == 0:
        conf_matrix_pixel[1,1] += 1
    if nppredMaskImg1s == 0 and npmaskImg1s > 0:
        conf_matrix_pixel[1,0] += 1
    if nppredMaskImg1s > 0 and npmaskImg1s == 0:
        conf_matrix_pixel[0,1] += 1

    #conf matrix liverby liver contours existance
    if predMaskContours > 0  and maskContours > 0:
        conf_matrix_contours[0,0] += 1
    if predMaskContours == 0 and maskContours == 0:
        conf_matrix_contours[1,1] += 1
    if predMaskContours == 0 and maskContours > 0:
        conf_matrix_contours[1,0] += 1
    if predMaskContours > 0 and maskContours == 0:
        conf_matrix_contours[0,1] += 1

# metrics concerning whole image, if tum and liver correctly detected
TP = conf_matrix_pixel[0,0]
FP = conf_matrix_pixel[0,1]
FN = conf_matrix_pixel[1,0]
TN = conf_matrix_pixel[1,1]
std_metrics = get_std_metrics(TP, TN, FP, FN)

f.write('\n')    
f.write('Image Level Ddetection ' + plot_title_preffix + ' totaly: , TP, FP, FN, TN, Accuracy, Precision, Recall, F1_score \n')
f.write(' ,'+ str(TP) + ', ' + str(FP)  + ', '  + str(FN) + ', ' + str(TN) +  ', ' + str(std_metrics["accuracy"]) + ', ' + str(std_metrics["precision"]) +', ' + str(std_metrics["recall"]) +', ' + str(std_metrics["f1_score"]) +'\n') 

       
#%%loop end

ax1 = sns.heatmap(conf_matrix_pixel, annot=True, cmap='Blues')
ax1.set_title(plot_title_preffix + ' detection by pixel existance (from CTs images)')
...
fig = plt.gcf()
plt.show(block=True)
fig.savefig(f'conf_matrix_{plot_title_preffix}_by_pixel.png')
plt.close()

ax2 = sns.heatmap(conf_matrix_contours, annot=True, cmap='Blues')
ax2.set_title(plot_title_preffix + ' detection by contours (from CTs images)')
...
fig = plt.gcf()
plt.show(block=True)
fig.savefig(f'conf_matrix_{plot_title_preffix}_by_contours.png')
plt.close()
