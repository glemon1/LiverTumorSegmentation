...
learn0 = unet_learner(dls, resnet50, metrics= [foreground_acc, cust_foreground_acc], self_attention=True, act_cls=Mish, opt_func=ranger) # initial case from fastai segm, best result so far
#learn0 = unet_learner(dls, resnet50, loss_func=CrossEntropyLossFlat(axis=1), metrics=[foreground_acc, cust_foreground_acc])  # init liver segm from kaggle, not so good
#learn0 = unet_learner(resnet34)
learn0 = learn0.load('Liv_n_tum_segm_resnet50_40ep_CrossEntropyLossFlat') ####//////////////////////////////////////////////////////////////////////////////////
...
for i in range(loop_rerpeats):
    ...
    for pred_no in range(batch_items):
        ...
        testMaskImg = cv2.imread("./test_masks/" + testMaskFile, 0)  #test masks with liver
        ...
        if (np.any(a) and SAVE_MASK_PRED_FILES): # print/save masks with liver only else no save zero mask, 
            ...
            pred_mask_img.save(f"pred_masks_Liv_n_tum_segm_resnet50_40ep_CrossEntropyLossFlat/{testMaskFile}") ###/////////////////////////////////////////////////////////////////////
...
ax.set_title('Liver and lession segmentation results\n\n');
