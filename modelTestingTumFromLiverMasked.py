...
codes = np.array(["background","tumor"])
...
dls = camvid.dataloaders(path/'train_images_liver_masked', bs=8)

learn0 = unet_learner(dls, resnet34, self_attention=True, act_cls=Mish, opt_func=ranger)

#learn0 = unet_learner(resnet34)
learn0 = learn0.load('tumor_segmentation_from_liver_masked_resnet34ep100')
...
#%%loop start
path_test_im = './test_images_liver_masked'
...
for i in range(loop_rerpeats):
    ...
    for pred_no in range(batch_items):
        ...
        testMaskImg = cv2.imread("./test_masks_tumor/" + testMaskFile, 0)  #test masks with liver
        ...
        if (np.any(a) and SAVE_MASK_PRED_FILES): # print/save masks with liver only else no save zero mask, 
            ...
            pred_mask_img.save(f"pred_masks_tumor_from_liver_masked_100ep/{testMaskFile}")
...
ax.set_title('Tumor segmentation from liver masked\n');
ax.set_xlabel('\nActual Values for tumor')
ax.set_ylabel('Predicted Values for tumor existence');
